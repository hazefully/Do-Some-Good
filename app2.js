...
sessionTest.new_entry = new location.model({sender_id: '123456789'});
...
function processPostback(event, sessionObj) {
  ...
  console.log(payload);
  if(sessionObj.viewFlag) {
  	loaction.query(sessionObj, listEntries);
  }
  ...
  else if (payload == "ListEntries" || payload == "ViewMore"){
  	var messageData = {
		recipient: {
			id: senderId
		},
	...
	sessionObj.save();
  	...
  	var newLocation = new location.model(sessionObj.new_entry);
...
function listEntries(sessionObj, list)
{
  var senderId = sessionObj.sender_id;
  var offset = sessionObj.offset;
  ...
  for(var i = 0 ; i + offset < list.length && i < 4 ; i++)
  {
...
function createNewEntry(event, sessionObj)
{
  ...
  {
  	  sessionObj.new_entry = new location.model({sender_id: senderId});
